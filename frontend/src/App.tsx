...
const App: React.FC = () => {
  ...
  const gameRef = useRef(game); // Ref para acceso síncrono en onDrop
  ...
  // Análisis inicial: se lanza después del primer paint para evitar cascading renders
  useEffect(() => {
    const initialFen = gameRef.current.fen();
    const initialCoach = coachId;
    setTimeout(() => fetchAnalysis(initialFen, initialCoach), 0);
  ...
  const onDrop = (sourceSquare: string, targetSquare: string): boolean => {
    try {
      ...
      if (move) {
        ...
        // Actualizar refs de forma síncrona ANTES del render
        gameRef.current = gameCopy;
        historyRef.current = newHistory;
        currentStepRef.current = newStep;

        // Luego actualizar el estado para re-render
        setGame(gameCopy);
  ...
  const navigate = (dir: number) => {
    const nextStep = currentStepRef.current + dir;
    if (nextStep >= 0 && nextStep < historyRef.current.length) {
      const nextFen = historyRef.current[nextStep];
  ...
  const sendChat = async () => {
    ...
    const updatedLog = [...chatLog, { role: 'user', text: q }];
